...
exibe_resultado(RecomendacoesOrdenadas) :-
    nl, write('*** RECOMENDAÇÕES FINAIS ***'), nl,
    write('As trilhas recomendadas em ordem de compatibilidade são:'), nl,
...
exibe_ranking(Posicao, [Pontuacao-Trilha | Resto]) :-
    ...
    write('----------------------------------------------------'), nl,
    write('POSIÇÃO '), write(Posicao), write(': '), write(Trilha), write(' (Pontuação: '), write(Pontuacao), write(')'), nl,
...
justificar_recomendacao(Trilha, Pontuacao) :-
    ...
    write('   - Justificativa ('), write(Pontuacao), write(' pontos):'), nl,
    
    (
        HabilidadesPontuadas = [] ->
            write('     Nenhuma afinidade direta encontrada nas suas respostas.'), nl
        ;
            write('     Você pontuou nesta trilha porque demonstrou afinidade com:'), nl,
            escreve_habilidades(HabilidadesPontuadas)
    ).

escreve_habilidades([]).
escreve_habilidades([Habilidade | Resto]) :-
    write('       -> '), write(Habilidade), nl,
